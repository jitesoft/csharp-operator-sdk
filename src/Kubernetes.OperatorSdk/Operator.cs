...
public class Operator : Disposable, IOperator
{
    ...
    public Operator(OperatorConfiguration configuration, IKubernetes client, ILoggerFactory loggerFactory = null)
    {
        ...
        _logger.LogInformation("Operator SDK Version {Version}",
            typeof(Operator).Assembly.GetName().Version!.ToString(3));
    }
    ...
    public IOperator AddController<R>(IController<R> controller, string watchNamespace = "default",
        string labelSelector = null) where R : CustomResource
    {
        if (IsDisposed)
        {
        ...
        _logger.LogDebug(
            $"Added controller {controller} on namespace {(string.IsNullOrEmpty(watchNamespace) ? "\"\"" : watchNamespace)}");
    ...
    public IController AddControllerOfType<TController, TResource>() where TController : IController<TResource>
